...
voice.convert() {
  ...
  if [[ ! -z ${message} ]]; then
    ...
    docker run -it --rm -e "MESSAGE=$message" -v /tmp/voice/:/data -w /data ozzyjohnson/tts bash -c 'export; echo "${MESSAGE}" > text.txt; cat text.txt; espeak -f text.txt --stdout > voice.ogg'
