...
list.search() {
	...
	if [[ "${callback_query_data[$id]}" == "btn_trip_outrosX" ]]; then
		regex='❌'
	elif [[ "${callback_query_data[$id]}" == "btn_trip_outrosV" ]]; then
		regex='✅'
	fi

	if [[ "${callback_query_data[$id]}" =~ btn_trip_outros. ]] ; then
		while read line; do
	...
	if [[ "${callback_query_data[$id]}" == "btn_trip_comprarX" ]]; then
		regex='❌'
	elif [[ "${callback_query_data[$id]}" == "btn_trip_comprarV" ]]; then
		regex='✅'
	fi

	if [[ "${callback_query_data[$id]}" =~ btn_trip_comprar. ]] ; then
		while read line; do
	...
	if [[ "${callback_query_data[$id]}" == "btn_trip_passagensX" ]]; then
		regex='❌'
	elif [[ "${callback_query_data[$id]}" == "btn_trip_passagensV" ]]; then
		regex='✅'
	fi

	if [[ "${callback_query_data[$id]}" =~ btn_trip_passagens. ]] ; then
		while read line; do
	...
	if [[ "${callback_query_data[$id]}" == "btn_trip_tremX" ]]; then
		regex='❌'
	elif [[ "${callback_query_data[$id]}" == "btn_trip_tremV" ]]; then
		regex='✅'
	fi

	if [[ "${callback_query_data[$id]}" =~ btn_trip_trem. ]] ; then
		while read line; do
...
trip.cities() {
	...
	if [[ ${city} == "Madri" ]]; then
            ...
            trip.btn_GRU
            message="*Pagamento do hostel está ok*"
			ShellBot.sendMessage --chat_id ${message_chat_id[$id]} --text "$(echo -e ${message})" --parse_mode markdown
